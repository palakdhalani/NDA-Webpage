...
const Navbar = ({ onMenuClick }) => {
  ...
  const isSuper = location.pathname.includes('/superadmin');
  const roleDisplay = isSuper ? 'Super Admin' : 'System Admin';
  ...
  const getTitle = (path) => {
    ...
    if (!last || last === 'admin' || last === 'superadmin') return 'Dashboard';
    return last.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
  ...
  const alertsPath = isSuper ? '/superadmin/alerts#alerts' : '/admin';
  const profilePath = isSuper ? '/superadmin/profile' : '/admin';

  return (
    <header className="h-20 flex items-center justify-between px-6 md:px-8 bg-transparent gap-4 font-sans shrink-0">
      {/* Mobile Menu Button */}
      <button
        onClick={onMenuClick}
      ...
      {/* Mobile Title */}
      <h1 className="md:hidden text-2xl font-bold text-slate-800 tracking-tight line-clamp-1">
      ...
      <div className="flex-1 max-w-md hidden md:block">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder={`Search ${isSuper ? 'enterprise' : 'HRMS'} ledger...`}
            className="w-full h-12 pl-12 pr-4 bg-white rounded-2xl border border-slate-100 shadow-md focus:ring-2 focus:ring-blue-500/10 outline-none text-sm text-slate-600 placeholder:text-slate-400 transition-all font-medium"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6 ml-auto">
            ...
            {showNotifications && (
              <>
                ...
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-72 sm:w-80 bg-white rounded-2xl shadow-md border border-slate-100 z-20 py-2 overflow-hidden"
                >
                  ...
                  <div className="px-4 py-2 border-t border-slate-50">
                    <button 
                      onClick={() => { navigate(alertsPath); setShowNotifications(false); }}
        ...
        <div className="relative">
          ...
          >
            <div className="w-10 h-10 rounded-full md:rounded-xl overflow-hidden shadow-inner border border-slate-50 shrink-0">
              <img
                src={profilePhoto}
                alt="Admin"
            ...
            <div className="hidden md:block">
              <h4 className="text-sm font-bold text-slate-800 leading-tight">{userName}</h4>
              <p className="text-[11px] text-slate-400 font-medium">{roleDisplay}</p>
            </div>
